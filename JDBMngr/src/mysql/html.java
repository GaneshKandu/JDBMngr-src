...
"        <p><input type=\"text\" name=\"port\" value=\"3306\" placeholder=\"Port\"></p>\n" +
...
"  </div>"
   ...
   public String querycssjs(){
       html = "";
       html = "<link rel=\"stylesheet\" type=\"text/css\" href=\"css/TextareaDecorator.css\">"
               + "<link rel=\"stylesheet\" type=\"text/css\" href=\"css/style.css\">"
               + "<script src=\"js/Parser.js\" type=\"text/javascript\"></script>"
               + "<script src=\"js/TextareaDecorator.js\" type=\"text/javascript\"></script>"
...
"		</script>";
   ...
   public String tab(String tab,String ctab){
        html = "";
        if((ctab+"_").equals(tab+"_")){
            html = " class=\"selected\" ";
        }
       return html;
   }   
   public String poperror(String error){
        if(error != ""){
            error = "<script>dialogbox(\"MySQL ERRORS\",\""+error+"\");</script>";
        }
        return error;
   }
...
"	}catch(e)\n" +
"	{\n" +
"		dialogbox(\"File Upload ERRORS\",\"Error: seems File API is not supported on your browser\");\n" +
"	    return;\n" +
"	}  \n" +
...
"  document.getElementById(\"query\").value = fileString;\n" +
"}\n" +
"\n" +
...
"	{\n" +
"		dialogbox(\"File Upload ERRORS\",\"Error reading file...\");\n" +
"	}\n" +
"}\n" +
"		</script>";
       return html;
...
public String getpagination(){
    html = "";
    int i = 0;
    int last = (b.getRowCount()/100);
    int rem = (b.getRowCount()%100);
    if(rem == 0){
        last -= 1;
    }
    int pg = b.getPage();
    String table = b.getTable();
    String db = b.getDb();
    if(pg >= 4){i = pg - 3;}else{i=1;}
    if(i > (last-7)){i = last-7;}
    if(last > 1){
        html = "<ul id=\"pagination\">\n";
        html += "<li><a href=\"index.jsp?db="+db+"&table="+table+"\">First</a></li>\n";
        int l = i+7;
        for(int x=i;(x<l && x<last);x++){
            i = x;
            if(i>0){
                html += "<li><a href=\"index.jsp?db="+db+"&table="+table+"&pg="+(i)+"\">"+(i)+"</a></li>\n";
            }
        }
        html += "<li><a href=\"index.jsp?db="+db+"&table="+table+"&pg="+last+"\">last</a></li>\n";
        html += "</ul>";
    }
    return html;
 }
